...
# HOG parameters کم‌حجم‌تر
HOG_ORIENTATIONS = 9
HOG_PIXELS_PER_CELL = (16, 16)  # قبلاً 8x8 بود
...
def extract_hog_features(images):
    hog_features = []
    for img in images:
        img_reshaped = img.reshape(64,64)  # مطابق preprocess.py
    ...
    return np.array(hog_features, dtype='float32')  # کاهش حافظه
...
def extract_lbp_features(images):
    ...
    return np.array(lbp_features, dtype='float32')
...
def extract_features(images):
    ...
    features = np.concatenate([hog_feat, lbp_feat], axis=1).astype('float32')
